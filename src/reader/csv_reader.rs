        ...
        for (i, header) in headers_clone.iter().enumerate() {
            let field_val = if let Some(field) = record.get(i) {
                if field.is_empty() {
                    ...
                    if let Ok(i_val) = field.parse::<i64>() {
                        serde_json::Value::Number(i_val.into())
                    } else if let Ok(f_val) = field.parse::<f64>() {
                        serde_json::Value::Number(serde_json::Number::from_f64(f_val).unwrap_or(serde_json::Number::from(0)))
...
pub fn read_csv_data(file_path: &Path, head: Option<usize,>,) -> Result<CsvData, DataReaderError,> {
    ...
    let first_lines: Option<Vec<String>> = if num_lines_to_extract > 0 {
        let cloned_file = file.try_clone().map_err(|e| DataReaderError::FileReadError {
            path: file_path.to_path_buf(),
            source: e,
        })?;
        let decoder = crate::reader::charset::get_decoded_reader(cloned_file).map_err(|e| DataReaderError::FileReadError {
            path: file_path.to_path_buf(),
            source: e,
        })?;
            ...
            for (header, value) in obj {
                let current_type = match value {
                    ...
                    serde_json::Value::Number(n) => {
                        if n.is_i64() { DataType::Integer } else { DataType::Float }
                    }
                    serde_json::Value::String(_) => DataType::String,
                    _ => DataType::Unknown,
                };
                schema_map.entry(header.clone())
